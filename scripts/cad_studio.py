    ...
    if args.command in {"retry", "cancel"}:
        ...
        if args.command == "retry":
            if job.get("status") not in {"failed", "cancelled", "blocked", "review_required"}:
                raise SystemExit(f"任务当前状态不可重试: {job.get('status')}")
            job.update({"status": "queued", "progress": 0, "updatedAt": now, "lastMessage": "已重新排队，等待 worker 执行。"})
            job.pop("error", None)
            job.pop("blockedReasons", None)
